euclideanDistance <- function(u, v) sqrt(sum(u - v)^2)

weightsKWNN = function(i, k)  (k + 1 - i) / k

sortObjectByDist <- function(xl, z)
{
	l <- dim(xl)[1]
	n <- dim(xl)[2] - 1
	distances <- rep(0, l)
	for (i in 1:l)
  	{
    		distances[i] <- c(euclideanDistance(xl[i, 1:n], z))
  	}
...
{  
	...
	for(i in 1:k)
	{
		weights[classes[i]]<-weights[classes[i]]+weightsKWNN(i,k);
  	}
